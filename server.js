...
//   const updateQuery = `
//     UPDATE srifincredit_views.srifin_loan_details_cutoffdate
//     SET cutoff_date = DATE '${cutoff_date}'`;
...
//       "Dummy2_ACT",
// 	    "Dummy2_ACT"
// 	    FROM srifincredit_views.vw_cic_base_data
//       WHERE CAST(cycledate AS DATE)= DATE '${cutoff_date}' AND 
//       CAST(disb_date AS DATE) BETWEEN DATE '${fromDate}' AND DATE '${toDate}' `;
...
//   const query = `
//     SELECT header, trail 
//     FROM srifincredit_views.vw_srifin_cic_report_types 
...
//     const header = result[1].column1 || '';
//     const trail = result[1].column2 || '';
...
app.post("/generate-report", async (req, res) => {
  ...
  try {
    ...
    const selectQuery = `SELECT * FROM srifincredit_views.vw_cic_base_data WHERE CAST(cycledate AS DATE) = DATE '${cutoff_date}' AND CAST(disb_date AS DATE) BETWEEN DATE '${fromDate}' AND DATE '${toDate}'`;
...
app.post("/generate-reupload", async (req, res) => {
  ...
  try {
    ...
    const formattedCsvData = csvData.slice(1).filter((val) => val.trim() !== "").join(",");
    const selectQuery = `SELECT * FROM srifincredit_views.vw_cic_base_data WHERE CAST(cycledate AS DATE) = DATE '${cutoff_date}' AND loan_application_id IN (${formattedCsvData})`;
...
app.post("/get-report-header-trailer", async (req, res) => {
  ...
  try {
    ...
    const { header = "", trail = "" } = result[0]; // Ensuring safe data retrieval
    res.json({ header, trail });
  } catch (error) {
